...
def is_windows_app_runtime_installed():
    ...
    """
    required_version = "1.8_8000.625.330"
    
    try:
        result = subprocess.run([
            "powershell", "-Command",
            "Get-AppxPackage -Name '*WindowsAppRuntime*' | Select-Object PackageFullName"
        ], capture_output=True, text=True, check=True)
        
        if result.stdout.strip() and "WindowsAppRuntime" in result.stdout:
            print("Found WindowsAppRuntime via Get-AppxPackage:")
            print(result.stdout)
            
            # Check if the version contains the required version number
            lines = result.stdout.strip().split('\n')
            for line in lines:
                if required_version in line:
                    print(f"[SUCCESS] Found required WindowsAppRuntime version {required_version}")
                    return True

            print(f"[ERROR] Found WindowsAppRuntime but not version {required_version}")
            return False
    except Exception as e:
        print(f"Error checking for WindowsAppRuntime: {e}")

    print(f"[ERROR] WindowsAppRuntime version {required_version} not found")
    return False
